...
type Player = {piece:number, x:number, z:number, x_change:number, z_change:number, speed:number, direction:string} //dir:"+x"/"-x"/"+z"/"-z"
...
const players = new Map<string, Player>(); //[key: id, value: {piece: letypedelapiece, x:current_x, z:current_z, x_change: Xchange_wanted, z_change: Zchange_wanted, speed:vitesse de deplacement}]
...
io.on("connection", (socket)=>{
    ...
    socket.on("desired_player_pos", desiredPos=>{
        ...
        if(!player)throw Error("no player in function desired player pos");
...
function gameLoop(){
    ...
    players.forEach((player1, key1)=>{
        ...
        players.forEach((player2, key2)=>{if(player1 != player2 &&Math.round(player2.x) == Math.round(player1.x) && Math.round(player2.z) == Math.round(player1.z)){
            let looser1 = null;
            let looser2 = null;
    ...
    if(accumulator > 2){
        let syncArray :{id:string, x:number, z:number, x_change:number, z_change:number, speed:number, direction:String}[] = [];
        players.forEach((value, key)=>{
