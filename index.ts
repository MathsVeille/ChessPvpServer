...
type Player = {piece:number, x:number, z:number, x_change:number, z_change:number, speed:number}
...
function gameLoop(){
    ...
    players.forEach((value, key1)=>{
        ...
        players.forEach((player, key2)=>{if(value != player &&Math.round(player.x) == Math.round(value.x) && Math.round(player.z) == Math.round(value.z)){
    ...
    if(accumulator > 2){
        let syncArray :{id:string, x:number, z:number, x_change:number, z_change:number}[] = [];
        players.forEach((value, key)=>{
